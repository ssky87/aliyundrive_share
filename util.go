...
func (d *AliyundriveShare) copyFile(shareId string, fileId string) (string, error) {
	...
	data := `{"requests":[{"body":{"file_id":"` + fileId + `","share_id":"` + id + `","auto_rename":true,"to_parent_file_id":"` + d.OpenConfig.MyStorePath + `","to_drive_id":"` + d.OpenConfig.MyDriveId + `"},"headers":{"Content-Type":"application/json"},"id":"0","method":"POST","url":"/file/copy"}],"resource":"file"}`

	var result BatchResult
	_, err := d.request("https://api.aliyundrive.com/adrive/v4/batch", http.MethodPost, func(req *resty.Request) {
